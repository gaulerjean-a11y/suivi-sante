...
function saveApiKey() {
  ...
  if (!key.startsWith('AI')) return showToast('Clé invalide — elle doit commencer par "AI"');
  localStorage.setItem('vitalis_gemini_key', key);
...
function getApiKey() {
  return localStorage.getItem('vitalis_gemini_key') || '';
}
...
function showApiKeyStatus(saved) {
  ...
  if (!el) return;
  if (saved || getApiKey()) {
    el.innerHTML = '<span class="api-key-ok">✅ Clé API configurée — IA activée</span>';
  } else {
    el.innerHTML = '<span class="api-key-missing">⚠️ Aucune clé — <a href="https://aistudio.google.com" target="_blank">Obtenir une clé gratuite</a></span>';
  }
}
...
async function analyzeWithAI() {
  ...
  if (!apiKey) {
    showToast('⚠️ Configure ta clé Gemini dans Profil');
    return;
  }
...
// Patch openAddFoodModal to reset AI state
const _origOpenAddFoodModal = openAddFoodModal;
function openAddFoodModal(meal = 'breakfast') {
  _origOpenAddFoodModal(meal);
  setTimeout(() => {
    const btn = document.getElementById('btn-ai-analyze');
    const status = document.getElementById('ai-status');
    if (btn) {
